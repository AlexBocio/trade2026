...
class S3Client:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Skip connection check during init to avoid blocking
        logger.info(f"S3Client initialized for {self.endpoint} (lazy connection)")
...
async def wait_for_s3(endpoint: str, timeout: int = 30, retry_interval: int = 5):
    """
    Wait for S3 endpoint to be available (async)
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout_obj) as session:
                async with session.get(endpoint) as response:
                    if response.status in [200, 403, 405]:  # S3 returns 200/403/405 depending on request
